...
export function HomeExperience() {
  ...
  return <>
    ...
    <MobileComingSoon />
    {user ? <aside className="sync-banner" role="status"><Icon name="user" width={22} height={22} /><div><strong>Compte connecté</strong><span>{syncStatus === "synced" ? "Votre progression locale et votre compte sont synchronisés." : syncStatus === "syncing" ? "Synchronisation de votre progression…" : syncStatus === "offline" ? "Hors ligne : vos changements restent enregistrés sur cet appareil." : syncStatus === "error" ? "La synchronisation sera retentée à votre prochaine connexion réseau." : "Votre progression reste disponible sur cet appareil."}</span></div></aside> : null}
    <AccountOnboarding />
  </>;
}
