...
export function QuestionReportButton({ questionId }: { questionId: string }) {
  ...
  return <>
    <button className="report-trigger" onClick={() => { setSent(false); setOpen(true); }} ref={triggerRef} type="button"><Icon name="flag" width={17} height={17} /> Signaler cette question</button>
    {open ? <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section aria-labelledby="report-title" aria-modal="true" className="report-modal" onKeyDown={handleKeyDown} ref={modalRef} role="dialog"><button aria-label="Fermer" autoFocus className="modal-close" onClick={() => setOpen(false)} type="button"><Icon name="close" width={20} height={20} /></button>{sent ? <div className="report-success"><span><Icon name="flag" width={26} height={26} /></span><h2 id="report-title">Merci pour votre signalement</h2><p>{user ? "Il a été enregistré et sera examiné." : "Il est conservé sur cet appareil et sera transmis si vous connectez ensuite un compte."}</p><button className="button" onClick={() => setOpen(false)} type="button">Fermer</button></div> : <><p className="eyebrow">Améliorer EXCI</p><h2 id="report-title">Signaler cette question</h2><p className="modal-help">Choisissez le problème constaté. Aucun compte n’est requis pour conserver votre signalement.</p><form action={(data) => void submit(data)}><label htmlFor={`reason-${questionId}`}>Motif</label><select defaultValue="ambiguous" id={`reason-${questionId}`} name="reason" required>{reasons.map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select><label htmlFor={`details-${questionId}`}>Précisions <span>(facultatif)</span></label><textarea id={`details-${questionId}`} name="details" rows={4} maxLength={1000} placeholder="Expliquez brièvement le problème…" /><button className="button" disabled={sending} type="submit">{sending ? "Envoi…" : "Envoyer le signalement"}</button></form></>}</section></div> : null}
  </>;
}
