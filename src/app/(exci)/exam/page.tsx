...
export default function ExamPage() {
  ...
  const { recordAttempt, recordExam } = useApp();
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      try {
        ...
        if (saved?.deadline && saved.deadline > Date.now() && saved.questionIds.length === 40) {
          ...
          if (restored.length === 40) { setExamQuestions(restored); setAnswers(saved.answers); setIndex(saved.index); setDeadline(saved.deadline); setRemaining(saved.deadline - Date.now()); setPhase("active"); }
  ...
  const finishExam = useCallback(() => {
    ...
    recordExam({ score, total: examQuestions.length });
    setFinalScore(score); setPhase("result"); localStorage.removeItem(examKey);
  }, [answers, examQuestions, recordAttempt, recordExam]);

  useEffect(() => {
    if (phase !== "active" || !deadline) return;
    const update = () => { const value = deadline - Date.now(); setRemaining(Math.max(0,value)); if (value <= 0) finishExam(); };
    update(); const timer = window.setInterval(update, 1000); return () => clearInterval(timer);
  }, [phase, deadline, finishExam]);

  function startExam() { const selected = shuffledQuestions(40); const end = Date.now() + duration; finishing.current = false; setExamQuestions(selected); setAnswers({}); setIndex(0); setDeadline(end); setRemaining(duration); setReview(false); setPhase("active"); }
  ...
  return <section className="practice-layout exam-active"><div className="practice-header exam-practice-header"><button className="exit-button" onClick={exitExam} type="button"><Icon name="close" width={19} height={19} /> Quitter</button><div className="progress-wrap"><div className="progress-label"><span>Examen blanc</span><strong>{answered} / 40 répondues</strong></div><div className="progress-bar"><i style={{width:`${answered/40*100}%`}} /></div></div><span className={remaining < 5*60*1000 ? "exam-timer urgent" : "exam-timer"}><Icon name="clock" width={18} height={18} />{formatTime(remaining)}</span></div><div className="question-navigator" aria-label="Navigation entre les questions">{examQuestions.map((item, itemIndex) => <button aria-label={`Question ${itemIndex+1}${answers[item.id] !== undefined ? ", répondue" : ""}`} className={`${itemIndex === index ? "current" : ""} ${answers[item.id] !== undefined ? "answered" : ""}`} key={item.id} onClick={() => setIndex(itemIndex)} type="button">{itemIndex+1}</button>)}</div><QuestionView question={question} selected={answers[question.id] ?? null} validated={false} onSelect={(selected) => setAnswers((current) => ({...current,[question.id]:selected}))} /><div className="exam-actions"><button className="button secondary small" disabled={index === 0} onClick={() => setIndex((value) => value-1)} type="button">Précédente</button>{index < 39 ? <button className="button small" onClick={() => setIndex((value) => value+1)} type="button">Suivante <Icon name="arrow" width={17} height={17} /></button> : <button className="button small" onClick={() => { if(window.confirm(`Terminer l’examen avec ${answered} réponse${answered>1?"s":""} enregistrée${answered>1?"s":""} ?`)) finishExam(); }} type="button">Terminer l’examen</button>}</div></section>;
}
