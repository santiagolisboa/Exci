...
export default function ErrorsPage() {
  ...
  const chronological = [...progress.attempts].sort((a, b) => a.answeredAt.localeCompare(b.answeredAt));
  const latest = new Map(chronological.map((attempt) => [attempt.questionId, attempt]));
  const errors = [...latest.values()].filter(({ correct }) => !correct).sort((a, b) => b.answeredAt.localeCompare(a.answeredAt));
